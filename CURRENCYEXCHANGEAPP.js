...
const countryList = {
  ...
  const populateDropdowns = () => {
    for (const currency in countryList) {
      ...
      toCurrency.appendChild(optionClone);
    }
  };
  
  const updateFlag = (element, currency) => {
    element.src = `https://flagsapi.com/${countryList[currency]}/flat/64.png`;
  };
  ...
  form.addEventListener("submit", async (event) => {
    ...
    exchangeRateText.textContent = `${amount} ${from} = ${convertedAmount} ${to}`;
  });
